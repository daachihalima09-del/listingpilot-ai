...
import { ProductInput } from '@/components/workspace/ProductInput';
...
export default function HomePage() {
  ...
  const [truthRows, setTruthRows] = useState<TruthRow[]>(demoProduct.truthRows);
  ...
  const [specText, setSpecText] = useState(demoSpecs);
  const [useDemoFallback, setUseDemoFallback] = useState(false);
  const [supplierUrl, setSupplierUrl] = useState('');
  const [productUrl, setProductUrl] = useState('');
  const [selectedPdf, setSelectedPdf] = useState<File | null>(null);
  const [inputError, setInputError] = useState<string | null>(null);
  const [analysisContext, setAnalysisContext] = useState<DemoAnalysisContext | null>(null);
  const [activeProduct, setActiveProduct] = useState<DemoProduct>(demoProduct);
  const [listingContent, setListingContent] = useState(buildListingContent);
  ...
  const handleResolveConflict = () => {
    if (!activeProduct.conflict.label || !activeProduct.conflict.recommendedValue) {
      return;
    }
        ...
        <div className="min-w-0 flex-1 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-3 backdrop-blur">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold">ListingPilot AI</p>
                <p className="text-xs text-slate-400">Product truth workspace</p>
              </div>
            </div>
