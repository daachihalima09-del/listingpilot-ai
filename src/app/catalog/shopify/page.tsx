              ...
              <tbody>
                {result.products.map((product) => (
                  <tr key={product.id} className="border-t border-white/10">
                    ...
                    <td className="p-4"><div className="flex items-center gap-2"><Link className="rounded-lg border border-white/10 px-3 py-2" href={`/catalog/shopify/${encodeShopifyProductReference(product.id)}`}>Preview</Link>{product.importStatus.projectId ? <Link className="rounded-lg bg-emerald-400 px-3 py-2 font-semibold text-slate-950" href={product.importStatus.status === 'PROJECT_ARCHIVED' ? '/projects?archived=true' : `/workspace/${product.importStatus.projectId}`}>Open Existing Project</Link> : <ShopifyCatalogImportButton productId={product.id} />}</div></td>
                  </tr>
                ))}
              </tbody>
            </table>
