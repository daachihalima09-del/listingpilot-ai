...
export async function POST(request: Request, context: { params: Promise<{ projectId: string; productId?: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await executeSafePublishingPlan(user.id, productId ?? projectId, await readBoundedJsonRequest(request, 32 * 1024), productId ? projectId : undefined));
