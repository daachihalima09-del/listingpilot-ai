  ...
  async function addProduct(event: FormEvent) {
    ...
    try {
      const response = await projectApiRequest<{ product: Summary }>(
        ...
        { method: 'POST', body: { workspaceId, name } },
      );
  ...
  async function renameProduct(event: FormEvent, product: Summary) {
    ...
    try {
      const response = await projectApiRequest<{ product: Summary }>(
        ...
        { method: 'PATCH', body: { workspaceId, version: product.version, name: renameValue } },
      );
      ...
      <section className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-slate-400">Products are independent workspaces with their own listing and Shopify state.</p>
          </div>
          {canManage && (
        ...
        {adding && (
          <form onSubmit={addProduct} className="mb-6 flex flex-wrap gap-3 rounded-2xl border border-slate-700 bg-slate-900/70 p-5">
          ...
          {products.length === 0 ? (
            ...
            <div key={product.id} className="grid grid-cols-[40px_minmax(220px,2fr)_repeat(4,minmax(100px,1fr))_auto] items-center gap-4 border-b border-slate-800 px-5 py-5 last:border-b-0">
              ...
              <div>{renamingId === product.id ? (
                <form onSubmit={(event) => renameProduct(event, product)} className="flex gap-2">
                  ...
                  <button type="submit" className="rounded-lg bg-amber-300 px-3 py-2 text-sm font-semibold text-slate-950">Save</button>
                </form>
              ) : <><p className="font-semibold">{product.name}</p><p className="mt-1 text-xs text-slate-500">Updated {new Date(product.updatedAt).toLocaleDateString()}</p></>}</div>
              ...
              <div className="flex gap-2">
                {canManage && <button type="button" onClick={() => { setRenamingId(product.id); setRenameValue(product.name); }} className="rounded-lg border border-slate-700 px-3 py-2 text-sm hover:border-amber-300">Rename</button>}
