...
export function MerchantListingProfileForm({ workspaceId, initial }: { workspaceId: string; initial: { version: number; data: ListingPreferenceData } }) {
  ...
  if (data.learningMode === 'LEARN_FROM_STORE') {
    return <div className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.07] p-6"><Sparkles className="h-5 w-5 text-amber-200" /><h2 className="mt-3 text-lg font-semibold text-white">Store learning is pending</h2><p className="mt-2 text-sm leading-6 text-slate-300">ListingPilot will learn from your Shopify catalog in a future analysis sprint. No rules or Shopify data are changed today.</p><button type="button" onClick={() => router.push(`/projects/new?${new URLSearchParams({ workspaceId })}`)} className="mt-5 rounded-xl bg-amber-300 px-4 py-2.5 text-sm font-semibold text-slate-950">Continue</button></div>;
  }
  ...
  async function save() {
    ...
    try {
      ...
      router.push(`/projects/new?${new URLSearchParams({ workspaceId })}`);
