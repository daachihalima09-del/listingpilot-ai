...
export function selectGenerationFacts(input: FactSelectionInput): Readonly<{ facts: readonly GenerationFact[]; reviews: readonly GenerationReviewRequirement[] }> {
  ...
  const facts = input.findings.map((finding) => {
    ...
    const resolvedStatus = statusFor({ truthStatus: finding.status, aiStrictness: input.aiPolicy.factualStrictness, aiAuthorityOnly: finding.evidenceSummary.strongestAuthority === 'AI_DERIVED', highRisk, evidenceStrong });
    const soleVariantId = input.product.variants.length === 1 ? input.product.variants[0]?.id : null;
    const variantScopeIsSafe = !finding.variantId || finding.variantId === soleVariantId;
    const selectionStatus = variantScopeIsSafe ? resolvedStatus : 'EXCLUDED';
    ...
    const reviewType = !variantScopeIsSafe ? 'FACT_REVIEW' : selectionStatus === 'CONFLICTED' ? 'CONFLICT_REVIEW' : finding.status === 'MERCHANT_OVERRIDE' ? 'MERCHANT_OVERRIDE_REVIEW' : highRisk && selectionStatus === 'REVIEW_REQUIRED' ? 'HIGH_RISK_REVIEW' : selectionStatus === 'REVIEW_REQUIRED' ? 'FACT_REVIEW' : null;
    if (reviewType) reviews.push({ id: `generation_review_${hasher.hash({ reviewType, id })}`, type: reviewType, priority: reviewType === 'CONFLICT_REVIEW' || reviewType === 'HIGH_RISK_REVIEW' ? 'CRITICAL' : 'HIGH', blocking: false, fieldIds: [fieldId], reason: variantScopeIsSafe ? finding.explanation : 'A variant-specific fact was omitted because this Product contains multiple variants.', relatedIssueIds: finding.associatedIssueIds, relatedFactIds: [id], relatedProfileSection: reviewType === 'HIGH_RISK_REVIEW' ? 'ai' : null, resolutionOptions: ['OMIT', 'REVIEW_EVIDENCE', 'APPROVE_TRACEABLE_VALUE'], metadata: { safelyOmittedFromGeneration: true, variantScopeSafe: variantScopeIsSafe } });
